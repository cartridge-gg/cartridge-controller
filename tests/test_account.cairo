...
func test_upgrade{syscall_ptr: felt*, range_check_ptr, pedersen_ptr: HashBuiltin*}() {
    ...
    %{
        ...
        ids.initial_implementation = declare("./src/account/PluginAccount.cairo").class_hash
        ids.fake_implementation = declare("./tests/utils/FakeAccount.cairo").class_hash
        signer_hash = declare("./lib/starknet_plugin_account/contracts/plugins/signer/StarkSigner.cairo").class_hash
        ids.account_address = deploy_contract("./src/upgrade/Proxy.cairo", [ids.initial_implementation, get_selector_from_name('initialize'), 3, signer_hash, 1, 420]).contract_address
    ...
    IUpgradableAccount.upgrade(account_address, 0xdead);

    // Successfully upgrades to fake account that masquerades as ERC165.
    IUpgradableAccount.upgrade(account_address, fake_implementation);
